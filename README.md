...
- **🔧 Silent ROOT Install** - Тихая установка APK через `su` команды без диалогов
- **📡 Update Command** - Обработка команды `update_agent` от сервера
- **🏗️ Enhanced UpdateManager** - Проверка ROOT доступа и fallback на стандартную установку
- **✅ ROOT Detection Fix** - Исправлена критическая ошибка инициализации ROOT прав
- **🎮 Control Commands Fix** - Все кнопки управления теперь работают корректно
...
### Fleet Management (v1.9.2+)
- **🔄 OTA Updates** - Автоматическое скачивание и установка обновлений
- **🔧 Silent Install** - Тихая установка через ROOT (`pm install -r -d`) без диалогов
- **📡 Remote Commands** - Команда `update_agent` от сервера для принудительного обновления
...
- **✅ ROOT Fix** - Исправлена критическая ошибка определения ROOT прав при подключении
